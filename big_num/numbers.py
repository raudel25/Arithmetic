...
class BigNum(ArithmeticMath):
    def __init__(self, precision=20, base10: int = 1):
    ...
    def number1(self):
        return Numbers('1', True, self.__precision, self.__base10)
...
class Numbers:
    def __init__(self, number: str | float | list, positive: bool, precision: int, base10: int):
        ...
        self.__abs = self if positive else Numbers(number, True, precision, self.__base10)
    ...
    def __create_value_number(self, part_int: str, part_decimal: str):
        ...
        for i in range(len(part_decimal) // self.__base10):
            number_value[-i - 2] = int(part_decimal[i * self.__base10:self.__base10 * (i + 1)])

            if i + 1 == self.precision:
                break

        number_value[-1] = int(part_int[len(part_int) - self.__base10:len(part_int)])

        for i in range(1, len(part_int) // self.__base10):
            number_value.append(int(part_int[(-1 - i) * self.__base10:-i * self.__base10]))
    ...
    def __str__(self) -> str:
        ...
        for i in range(self.__precision):
            ...
            part_decimal = f'{add_zeros_left(aux, self.__base10 - len(aux))}{part_decimal}'
        for i in range(self.__precision, len(self.__number_value)):
            aux: str = str(self.__number_value[i])
            part_int = f'{add_zeros_left(aux, self.__base10 - len(aux))}{part_int}'
    ...
    def __add__(self, o: 'Numbers') -> 'Numbers':
        ...
        if x.sign == y.sign:
            return Numbers(sum_number(lx, ly, self.__base10), x.positive, self.__precision, self.__base10)
        compare = x.abs.compare_to(y.abs)
        if compare == 0:
            return self.real0()
        if compare == 1:
            return Numbers(sub_number(lx, ly, self.__base10), x.positive, self.__precision, self.__base10)

        return Numbers(sub_number(ly, lx, self.__base10), y.positive, self.__precision, self.__base10)

    def __sub__(self, o: 'Numbers') -> 'Numbers':
        return self + (-o)

    def __neg__(self) -> 'Numbers':
        return Numbers(self.__number_value, not self.positive, self.__precision, self.__base10)
    ...
    def __mul__(self, o: 'Numbers'):
        ...
        if x.abs == self.real1():
            return Numbers(y.__number_value, positive, y.__precision, self.__base10)
        if y.abs == self.real1():
            return Numbers(x.__number_value, positive, x.__precision, self.__base10)
    ...
    def __truediv__(self, o: 'Numbers'):
        ...
        if y.abs == self.real1():
            return Numbers(x.__number_value, positive, self.__precision, self.__base10)

        return Numbers(Numbers.newton_raphson(x, y).__number_value, positive, self.__precision, self.__base10)
    ...
    def newton_raphson(x: 'Numbers', y: 'Numbers'):
        ...
        a = Numbers(48 / 17, True, x.__precision, x.__base10) - Numbers(32 / 17, True, x.__precision, x.__base10) * y
    ...
    @property
    def cant_int(self):
        return len(str(self.__number_value[-1])) + (len(self.__number_value) - 1 - self.__precision) * self.__base10
    ...
    def ship_by_10(self, cant: int):
        ...
        if cant < 0:
            return Numbers(f'{s[0][:len(s[0]) - abs(cant)]}.{s[0][len(s[0]) - abs(cant):]}{s[1]}', self.positive,
                           self.precision, self.__base10)

        return Numbers(f'{s[0]}{s[1][:abs(cant)]}.{s[1][abs(cant):]}', self.positive, self.precision, self.__base10)
